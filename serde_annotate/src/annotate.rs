        ...
        pub fn cast<'a>(typename: &str, object: &AnyPointer<'a>) -> Option<&'a dyn Annotate> {
            if object.is_null() {
                ...
                let a = Self::lookup(typename).map(|cast| unsafe {
                    // If we found the type, its safe to cast it to dyn Annotate.
                    // Cast the object and use transmute to re-attach
                    // the lifetime 'a to the result.
